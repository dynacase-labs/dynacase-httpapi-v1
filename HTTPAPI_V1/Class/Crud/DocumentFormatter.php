...
class DocumentFormatter
{
    ...
    static protected $uselessProperties
        = array(
            "lockdomainid",
            "domainid",
            "svalues",
                ...
                if ($document->defDoctype === "C") {
                    ...
                    if ($document->doctype === "Z") {
                        ...
                        if ($document->locked == -1) {
                            return URLUtils::generateURL(sprintf("documents/%s/revisions/%d.json", $document->initid,
                                $document->revision));
                        } else {
    ...
    public function format()
    {
        ...
        $this->formatCollection->setAttributeRenderHook(function ($info, $attribute) {
            return $this->attributeHook($info, $attribute);
        });
        /** Add uri property and suppress state if no state **/
        $this->formatCollection->setDocumentRenderHook(function ($values, \Doc $document) {
    ...
    /**
     * Hook applied for each documents data
     * @param  array    $values
     * @param \Doc $document
     *
     * @return mixed
    ...
    /**
     * Hook applied for each attribute data
     * @param                  $info
     * @param \NormalAttribute $attribute
     *
     * @return array|null|\StandardAttributeValue
     */
    protected function attributeHook($info, $attribute)
    {
        if ($info === null) {
    ...
    protected function rewriteImageUrl(&$imgUrl)
    {
        ...
        if (preg_match($pattern, $imgUrl, $reg)) {
            $imgUrl = sprintf("%sdocuments/%d/images/%s/-1/sizes/%sx%sc.png", $this->rootPath, $reg[1], $reg[2],
                $reg[4], $reg[4]);
        }
    }
    ...
    protected function rewriteThumbUrl(&$imgUrl)
    {
        ...
        if (preg_match($pattern, $imgUrl, $reg)) {
            $imgUrl = sprintf("%sdocuments/%d/images/%s/%s/sizes/%s.png", $this->rootPath, $reg["docid"],
                $reg["attrid"], $reg["index"], $reg["size"]);
        }
    }
    ...
    protected function rewriteFileUrl(&$fileUrl)
    {
        ...
        if (preg_match($pattern, $fileUrl, $reg)) {
            $fileUrl = sprintf("%sdocuments/%d/files/%s/%s/%s", $this->rootPath, $reg["docid"], $reg["attrid"],
                $reg["index"], $reg["filename"]);
        }
    }
