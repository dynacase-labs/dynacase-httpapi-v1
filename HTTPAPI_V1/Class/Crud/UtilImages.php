...
class UtilImage
{
    public static function resizeLocalImage($sourceFileName, $dest, $size)
    {
        ...
        if (preg_match("/^([0-9]+x[0-9]+)c$/", $size, $reg)) {
            $cmd = sprintf("convert  -resize %s^ -gravity center -extent %s %s %s", escapeshellarg($reg[1]) , escapeshellarg($reg[1]) , escapeshellarg($sourceFileName) , escapeshellarg($dest));
        } else {
    ...
    public static function getVaultPath($vid, $onlyPublic = false)
    {
        ...
        if ($rcore) {
            ...
            $result = pg_query(sprintf("select id_dir,name,public_access,id_tmp from vaultdiskstorage where id_file = %s and ($publicCond id_tmp is null)", pg_escape_literal($vid)));
            if ($result) {
    ...
    public static function downloadFile($filePath, $fileName = "", $inline = true)
    {
        require_once ("WHAT/Lib.Http.php");
        if (!$fileName) {
            $fileName = basename($filePath);
        }
        Http_DownloadFile($filePath, $fileName, "", $inline, true);
    }
}
